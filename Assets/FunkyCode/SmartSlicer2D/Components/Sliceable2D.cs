...
namespace Slicer2D {
	...
	public class Sliceable2D : MonoBehaviour {
		...
		private Slicer2DSorter Slicer2DSort;
		private List<Collider2D> ColGroup;
		public ContactPoint2D contactPoint2D;
		...
		void OnEnable() {
			Slicer2DSort = gameObject.GetComponentInParent<Slicer2DSorter>();
		...
		public List<GameObject> PerformResult(List<Polygon2D> result, Slice2D slice) {
			...
			//Slicer2DSort.GroupParent.Clear();
			...
			#region body parts sorting after slicing
			if(resultGameObjects.Count >= 2)
			{
				foreach(GameObject bodyPart in resultGameObjects)
				{
					GetComponentInParent<Slicer2DSliced>().SlicedParts.Add(bodyPart);
					Rigidbody2D rigBody = bodyPart.GetComponent<Rigidbody2D>();
					if(rigBody != null)
					{
						Destroy(rigBody);
					}
			...
			if(resultGameObjects[0].GetComponent<Collider2D>().bounds.center.x < resultGameObjects[1].GetComponent<Collider2D>().bounds.center.x
			|| resultGameObjects[0].GetComponent<Collider2D>().bounds.center.y < resultGameObjects[1].GetComponent<Collider2D>().bounds.center.y)
			{
				resultGameObjects[0].transform.SetParent(Slicer2DSort.FirstSliceGroup);
				resultGameObjects[1].transform.SetParent(Slicer2DSort.SecondSliceGroup);
			}
			else
			{
				resultGameObjects[1].transform.SetParent(Slicer2DSort.FirstSliceGroup);
				resultGameObjects[0].transform.SetParent(Slicer2DSort.SecondSliceGroup);
			}
			}
			#endregion
